...
$app->register('db', $pdoClass, [$dsn, $_ENV['DB_USER'] ?? 'root', $_ENV['DB_PASSWORD'] ?? '']);

/**********************************************
 *              Latte Service                 *
 **********************************************/
$app->map('render', function (string $template, array $data, ?string $key = null): void {
	$latte = new Latte\Engine;

	// Où latte stocke spécifiquement son cache
	$latte->setTempDirectory(__DIR__ . '/../cache/');

	$finalPath = Flight::get('flight.views.path') . ($template[0] === '/' ? $template : '/' . $template);
	...
	// Render to string
	$output = $latte->renderToString($finalPath, $data);

	// If a key is provided, save to Flight view variables
	if ($key) {
		Flight::view()->set($key, $output);
	} else {
		echo $output;
	}
});
